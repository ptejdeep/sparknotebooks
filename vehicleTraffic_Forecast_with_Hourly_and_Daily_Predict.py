        ...
        for v_type, daily_count in daily_counts_by_type.items():
            ...
            for hour, count_in_hour in hourly_counts.items():
                for _ in range(count_in_hour):
                    ...
                    data.append({
                        'vehicleMovementstartdatetime': movement_time,
                        'vehicletype': v_type,
                        'vehicleentryexitlocationname': location,
                        'vehicletypeid': v_type_id,
                        'vehicleentryexitlocationid': loc_id,
                        'vehicleseqid': vehicleseqid_counter
                    })
                    vehicleseqid_counter += 1

    df = pd.DataFrame(data)
    df = df.sort_values(by='vehicleMovementstartdatetime').reset_index(drop=True)
    return df
...
print(f"Data period: {raw_df['vehicleMovementstartdatetime'].min()} to {raw_df['vehicleMovementstartdatetime'].max()}")

# COMMAND ----------

#output_base_path = "/Volumes/workspace/processed/files/"

#spark = SparkSession.builder.appName("JFKTrafficForecast").getOrCreate()

output_base_path = "/Volumes/workspace/processed/files/" # Your specified path

# Define the full path for your Parquet file
parquet_file_path = f"{output_base_path}vehicletrafficraw"
spark_df = spark.createDataFrame(raw_df)
print(f"\nSaving raw_df to Parquet at: {parquet_file_path}")
spark_df.write.mode("overwrite").parquet(parquet_file_path)

print("Raw data saved successfully in Parquet format.")
...
def create_traffic_forecast_model(df: pd.DataFrame):
    ...
    df['vehicleMovementstartdatetime'] = pd.to_datetime(df['vehicleMovementstartdatetime'])
    df.set_index('vehicleMovementstartdatetime', inplace=True)

    df_agg = df.groupby([
        pd.Grouper(freq='H'),
        'vehicletype',
        'vehicleentryexitlocationname'
    ...
    df_agg = df_agg.reindex(full_hourly_index, fill_value=0)
...
def create_traffic_forecast_model(df: pd.DataFrame):
    ...
    test_size_days = 30
    test_start_date = df_model.index.max() - timedelta(days=test_size_days)
    ...
    for i, date in enumerate(future_dates):
        ...
        current_step_df['weekofyear'] = date.isocalendar().week.astype(int)
        current_step_df['is_weekend'] = (date.dayofweek >= 5).astype(int)
        ...
        # This part can be complex and might need a circular buffer for historical predictions.
        # For simplicity, we'll try to get from combined_df, assuming it's reasonably filled.
        # A more robust solution for these would involve careful slicing of `combined_df`
        # or maintaining an expanding window of "actuals + predictions".
        if (date - timedelta(hours=24)) in combined_df.index:
            current_step_df['Total_Vehicles_lag_24h'] = combined_df.loc[date - timedelta(hours=24)]['Total_Vehicles'] if (date - timedelta(hours=24)) <= last_known_date else future_predictions_list[i-24] if i>=24 else 0 # Simplified
        else:
            current_step_df['Total_Vehicles_lag_24h'] = 0 # Default if not found (e.g., at very start of series)

        if (date - timedelta(hours=168)) in combined_df.index:
            current_step_df['Total_Vehicles_lag_168h'] = combined_df.loc[date - timedelta(hours=168)]['Total_Vehicles'] if (date - timedelta(hours=168)) <= last_known_date else future_predictions_list[i-168] if i>=168 else 0 # Simplified
        else:
        ...
        # or calculate it based on actuals + previous predictions if within the window.
        # A more precise way would be to dynamically calculate it as predictions are made.
        current_step_df['Total_Vehicles_rolling_24h_mean'] = combined_df.loc[last_known_date]['Total_Vehicles_rolling_24h_mean'] if i==0 else future_predictions_list[-100:].mean() if len(future_predictions_list) >= 100 else 0 # Very simplified
        current_step_df['Total_Vehicles_rolling_7d_mean'] = combined_df.loc[last_known_date]['Total_Vehicles_rolling_7d_mean'] if i==0 else future_predictions_list[-500:].mean() if len(future_predictions_list) >= 500 else 0 # Very simplified

        current_step_df.fillna(0, inplace=True) # Fill any remaining NaNs

        # Predict
        pred = model.predict(current_step_df[features])[0]
        future_predictions_list.append(max(0, pred)) # Ensure no negative predictions
    ...
    return trained_model, full_forecast_df, evaluation_metrics, df_aggregated, X_test.index.min(), future_dates.min()

# Run the forecast model
trained_model, full_forecast_df, eval_metrics, df_aggregated, test_start_date_actual, forecast_start_date_actual = create_traffic_forecast_model(raw_df.copy())

if trained_model is not None:
    print("\n--- Visualizing Forecast ---")

    # --- Plotting 1: Overall Forecast (Matplotlib) ---
    plt.figure(figsize=(18, 8))
    plt.plot(full_forecast_df['Actual'].loc[df_aggregated.index.get_level_values(0).min():forecast_start_date_actual], label='Historical Actuals', color='blue', alpha=0.7)
    plt.plot(full_forecast_df['Predicted'].loc[test_start_date_actual:forecast_start_date_actual], label='Test Predictions', color='orange', linestyle='--', alpha=0.8)
    plt.plot(full_forecast_df['Predicted'].loc[forecast_start_date_actual:], label='Future Forecast', color='green', linestyle=':', alpha=0.9)
    plt.axvline(x=test_start_date_actual, color='red', linestyle='--', label='Training/Test Split')
    plt.axvline(x=forecast_start_date_actual, color='purple', linestyle='--', label='Forecast Horizon Start')

    plt.title('JFK International Airport Total Vehicle Traffic Forecast - Overall View')
    plt.xlabel('Date and Time')
    plt.ylabel('Number of Vehicles (Hourly)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()

    # --- Plotting 2: Zoomed-in Test Period (Matplotlib) ---
    plt.figure(figsize=(15, 6))
    plt.plot(full_forecast_df['Actual'].loc[test_start_date_actual:forecast_start_date_actual], label='Actual Total Vehicles (Test)', alpha=0.7)
    plt.plot(full_forecast_df['Predicted'].loc[test_start_date_actual:forecast_start_date_actual], label='Predicted Total Vehicles (Test)', linestyle='--', alpha=0.8)
    plt.title('JFK International Airport Total Vehicle Traffic Forecast - Test Period')
    plt.xlabel('Date and Time')
    plt.ylabel('Number of Vehicles (Hourly)')
    plt.legend()
    plt.grid(True)
    ...
    # --- Plotting 3: Day-wise Aggregation (Matplotlib) ---
    # Aggregate to daily sums for visualization
    daily_actual = full_forecast_df['Actual'].resample('D').sum()
    daily_predicted = full_forecast_df['Predicted'].resample('D').sum()

    plt.figure(figsize=(15, 6))
    plt.plot(daily_actual.loc[daily_actual.index.min():forecast_start_date_actual.date()], label='Actual Daily Total Vehicles', color='blue', alpha=0.7)
    plt.plot(daily_predicted.loc[test_start_date_actual.date():forecast_start_date_actual.date()], label='Predicted Daily Total Vehicles (Test)', color='orange', linestyle='--', alpha=0.8)
    plt.plot(daily_predicted.loc[forecast_start_date_actual.date():], label='Forecasted Daily Total Vehicles (Future)', color='green', linestyle=':', alpha=0.9)
    plt.axvline(x=test_start_date_actual.date(), color='red', linestyle='--', label='Train/Test Split')
    plt.axvline(x=forecast_start_date_actual.date(), color='purple', linestyle='--', label='Forecast Horizon Start')
    plt.title('JFK International Airport Total Vehicle Traffic Forecast - Daily Aggregation')
    plt.xlabel('Date')
    plt.ylabel('Number of Vehicles (Daily)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()

    # --- Plotting 4: Hourly Pattern for a Sample Day (Matplotlib) ---
    # Choose a sample day from the test set for a 24-hour pattern view
    sample_day = test_start_date_actual + timedelta(days=5) # Example: 5 days into the test period
    sample_day_data = full_forecast_df.loc[sample_day.date().strftime('%Y-%m-%d')]

    plt.figure(figsize=(12, 6))
    plt.plot(sample_day_data.index.hour, sample_day_data['Actual'], label='Actual', marker='o')
    plt.plot(sample_day_data.index.hour, sample_day_data['Predicted'], label='Predicted', linestyle='--', marker='x')
    plt.title(f'Hourly Vehicle Traffic Forecast for {sample_day.date()}')
    ...
    # --- Plotting 5: Average Hourly Pattern (Matplotlib) ---
    # This shows the typical daily pattern learned by the model
    df_actual_pattern = df_aggregated.loc[X_train.index.min():X_train.index.max()].groupby('hour')['Total_Vehicles'].mean()
    df_predicted_pattern = pd.DataFrame({'Predicted': trained_model.predict(X_train)}).set_index(X_train.index).groupby('hour')[0].mean() # Average hourly prediction on training set

    plt.figure(figsize=(12, 6))
    plt.plot(df_actual_pattern.index, df_actual_pattern.values, label='Average Actual Traffic (Training Data)', marker='o')
    plt.plot(df_predicted_pattern.index, df_predicted_pattern.values, label='Average Predicted Traffic (Training Data)', linestyle='--', marker='x')
    plt.title('Average Hourly Vehicle Traffic Pattern (from Training Data)')
    plt.xlabel('Hour of Day')
    ...
    # --- Plotting 6: Interactive Plotly Plot for Overall Forecast ---
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=full_forecast_df.index, y=full_forecast_df['Actual'],
                             mode='lines', name='Historical Actuals',
                             line=dict(color='blue')))
    fig.add_trace(go.Scatter(x=full_forecast_df.loc[test_start_date_actual:].index, y=full_forecast_df['Predicted'].loc[test_start_date_actual:],
                             mode='lines', name='Forecasted (Test & Future)',
                             line=dict(color='orange', dash='dash')))

    fig.add_vline(x=test_start_date_actual, line_width=2, line_dash="dot", line_color="red",
                  annotation_text="Train/Test Split", annotation_position="top right")
    fig.add_vline(x=forecast_start_date_actual, line_width=2, line_dash="dot", line_color="purple",
                  annotation_text="Forecast Start", annotation_position="top left")

    fig.update_layout(
        title_text='Interactive JFK Airport Total Vehicle Traffic Forecast',
        xaxis_rangeslider_visible=True,
        xaxis_title="Date and Time",
        yaxis_title="Number of Vehicles (Hourly)",
        height=600
    )
    fig.show()

    # --- Plotting 7: Interactive Plotly Plot for Hourly Pattern (Week Overview) ---
    # Let's take a sample week from the future forecast to show detailed hourly patterns
    sample_week_start = forecast_start_date_actual
    sample_week_end = sample_week_start + timedelta(days=7)

    # Filter data for the sample week
    week_forecast_data = full_forecast_df.loc[sample_week_start:sample_week_end]

    fig_hourly = go.Figure()
    fig_hourly.add_trace(go.Scatter(x=week_forecast_data.index, y=week_forecast_data['Predicted'],
                                     mode='lines+markers', name='Predicted Traffic',
                                     line=dict(color='green')))

    fig_hourly.update_layout(
        title_text=f'Predicted Hourly Vehicle Traffic for Week Starting {sample_week_start.strftime("%Y-%m-%d")}',
        xaxis_title="Date and Time",
        yaxis_title="Number of Vehicles",
        hovermode="x unified", # Shows data for all traces at a specific x-value on hover
        height=500
    )
    fig_hourly.show()


    # You can also visualize feature importance if using tree-based models
